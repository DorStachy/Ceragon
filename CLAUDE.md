...
- **Shared contracts** (transitional — no published package/monorepo yet): each deployable repo **vendors its OWN copy** of `@ceragon/shared-contracts` inside the repo and builds against it via `"@ceragon/shared-contracts": "file:./packages/shared-contracts"`. The production copies are `Ceragon-Intelligence/packages/shared-contracts/` (Intel build) and `Backend/packages/shared-contracts/` (Backend build) — Backend does **not** build against the root copy. The root `packages/shared-contracts/` here is only a local-workspace **reference/umbrella copy** in the personal `DorStachy/Ceragon` repo; it is **not a production build source**, so a missing/stale root copy has no prod impact. When you change a contract field, update it in the copy of **every repo that actually consumes it** (e.g. a Release-observation/gate hint used only by Intel needs only the Intel copy), and keep the root reference copy in sync only if you want the local Intel parity spec (`shared-contracts-parity.spec.ts`) to stay green — that spec self-compares in CI and only checks root-vs-Intel locally. See [Ceragon-Intelligence/README.md](Ceragon-Intelligence/README.md).
