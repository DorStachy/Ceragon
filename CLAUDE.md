...
- **Shared contracts**: a transitional mirror remains for legacy/non-AI contracts in `packages/shared-contracts/` and `Ceragon-Intelligence/packages/shared-contracts/`. For M4.7 AI security, approved `G-CONTRACT-AUTHORITY` Option A supersedes mirror lockstep: workspace `packages/shared-contracts/` is the sole portable source, Ceragon Intelligence is not a consumer, and agents must not hand-copy AI-security source into that mirror. Backend, Installers/browser, and Frontend receive digest-pinned generated projections only in P0-C03–C06 and integrate at P0-C07; until then their deployed contracts remain unchanged. See [the authority decision](docs/ai-security/decisions/m47/G-CONTRACT-AUTHORITY.md).
