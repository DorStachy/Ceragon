...
let run = spawnSync(process.execPath, ['ci/lib/claim-contract-guard.mjs'], { encoding: 'utf8' });
if (run.status !== 0) throw new Error(run.stderr || run.stdout);
const dir = mkdtempSync(join(tmpdir(), 'claim-contract-'));
const bad = join(dir, 'bad.md');
writeFileSync(bad, plan.replace('| FC-15 ', '| REMOVED '));
run = spawnSync(process.execPath, ['ci/lib/claim-contract-guard.mjs', bad], { encoding: 'utf8' });
