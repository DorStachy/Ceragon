...
- Manual/lab on the live endpoint (record in the evidence file): paste a synthetic AWS key into a Codex desktop thread (expect BLOCK with the new remedy copy), then send 'hello' in the SAME thread and confirm it completes and the daemon log shows a redact-then-sent decision carrying reason 'replayed-context:redact'. DEFEAT STEP: repeat on the pre-fix binary and confirm 'hello' still fails identically — the brief records this reproduction at 21:19:08/20/32, so an unreproducible control means the test environment is wrong.
