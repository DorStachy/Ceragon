...
SINGLE-ENDPOINT SAMPLE BIAS: `<ENDPOINT_HOSTNAME>` is the test tenant and its 816 decisions are heavily agent-driven and skewed by this very engagement's probing. State that limitation on the number rather than presenting it as a fleet measurement — the honesty discipline applies to our own metrics too. The private evidence already records that no second endpoint exists, so a broader sample is owner-gated and not something this spec can conjure.
