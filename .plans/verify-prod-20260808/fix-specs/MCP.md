...
MEASURED GROUND TRUTH (re-derived on the live box, not taken from FINDINGS.md):
  ~/.claude.json top-level `mcpServers` is EMPTY ({}), and projects["<WORKSPACE>"].mcpServers = { firecrawl: {type:"http", url:"https://mcp.firecrawl.dev/<key-segment>/v2/mcp"} }.
...
- `live box: ~/.claude.json top-level mcpServers == {} ; projects["<WORKSPACE>"].mcpServers == {firecrawl:{type:http,url:https://mcp.firecrawl.dev/<seg>/v2/mcp}}`
