...
- [x] **T1 Cross-check every file in every wave** against all 28 rows of
      `.plans/PARALLEL_EXECUTION_CONTRACT.md` section 2, not just the three the plan named. Produce the table.
- [x] **T2 Post a SEAM REQUEST for `internal/daemon/server.go`** (needed by W1 T2; contract line 58,
...
- [x] **T5 Check the W3 T3 `pr-checks.yml` append** against the legs P47 appended this same morning, so
...
- [ ] **T3 Publish the release manifest from the RELEASE pipeline, not the Backend image.** *Rewritten after attempting it: the original prescription cannot be executed honestly.* It said `Backend/Dockerfile` should copy a manifest "generated at build from the deploy SHA". A Docker build cannot know what the manifest requires: `components` is a `Record` over **eight** ECS services each needing `taskDefinitionArn`, `taskDefinitionRevision`, `imageDigest` and `envHash` (deploy-time AWS facts), and `artifacts` needs `cliArtifactSha256`, `msiArtifactSha256` and `installedBinarySha256` (agent-release facts) — `Backend/src/health/types/release-manifest.types.ts:26-42`. Generating it at image-build time means **fabricating eleven values**, which would make the drift report — whose only purpose is detecting drift — assert invented expectations. That is worse than the 503 it replaces.
      **The loader half is already done and needs nothing.** `release-manifest.service.ts:69-78` already probes `/app/release-manifest.json`, `dist/`, cwd and `/etc/devoid` with no env wiring, and the service's own comment records that the root cause was two-layered: the loader had one source, *and* nothing ever assembled the document. Only the second half remains. So: the **release/deploy pipeline** — which alone knows the task-definition revisions, image digests and artifact hashes — writes the manifest and mounts or bakes it at a path already probed. Backend code changes not at all.
      Server defect since ≤ 2026-08-23 (`cmd/devoid/release_check.go:254-262`). **Do not touch the doctor row** — it is `sb.unverified` at warning weight by deliberate design (`main.go:1275-1279`, `:1679`), which is correct. And **do not bake a partial manifest to clear the row**: a green provenance row over fabricated digests is precisely the class of defect this programme exists to end.
