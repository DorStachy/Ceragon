...
| D2 | **Advance the production org's rollout ring** from `SHADOW/0` to `CANARY` with cohort > 0 — *if* it is still at the default. Until then **zero runtime-integrity intents reach any endpoint** (F3b). | W0, W2 | Production posture change, per org |
| D3 | **Is `ENROLLMENT_MODE=deferred` a supported state?** W1 treats it as one and fixes it. The alternative is removing it from the MSI's own LaunchCondition message. | W1 | Product decision |
...
- [ ] **T1 Enrol.** `devoid setup enroll --token <T> --backend-url https://api.devoid.one`. Record whether `credentials.json` appears at `%ProgramData%\devoid\` (expected: yes).
